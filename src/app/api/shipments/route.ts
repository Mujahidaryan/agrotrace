...
export async function GET(req: NextRequest) {
  ...
  if (!isDatabaseAvailable()) {
    ...
    return NextResponse.json({ success: true, data: results, meta: { total: SHIPMENTS.length, returned: results.length } });
  }

  try {
    const [shipments, total] = await Promise.all([getShipments({ province, status, mode, is_export, product, limit }), getShipmentCount()]);
    return NextResponse.json({ success: true, data: shipments, meta: { total, returned: shipments.length } }, { headers: { 'Cache-Control': 's-maxage=30, stale-while-revalidate=60' } });
  } catch (err) {
    console.error('[GET /api/shipments]', err instanceof Error ? err.message : String(err));
    return NextResponse.json({ success: false, error: { code: 'QUERY_FAILED', message: 'Failed to retrieve shipments' } }, { status: 500 });
  }
}
